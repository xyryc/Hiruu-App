        ...
        <View className="bg-white rounded-t-3xl">
          ...
          <SafeAreaView edges={["bottom"]} className="px-5 py-7">
            ...
            <View className="items-center">
              ...
              >
                <Image source={data.img} contentFit="contain" style={{ height: 80, width: 55 }} />
              </View>
              <Text className="font-proximanova-semibold text-base text-primary dark:text-dark-primary mt-4">{data.title || 'Hard worker'}</Text>
              <Text className="mt-1.5 text-white px-4 py-2 rounded-full" style={{ backgroundColor: data.tagColor }} >{data.buttonTitle || 'Bronze'}</Text>
            </View>
            <View className="flex-row justify-between mt-4">
              <Text className="font-proximanova-semibold text-sm text-primary dark:text-dark-primary">Hours worked</Text>
              <Text className="font-proximanova-semibold text-primary dark:text-dark-primary">{data.time || "300hrs/ 500hrs"}</Text>
            </View>
                ...
                badchcard.map((item, index) => (
                  <View className="items-center" key={index}>
                    <View
                      ...
                      <Image source={item.img} contentFit="contain" style={{ height: 27, width: 18 }} />
                    </View>
                    <Text className="font-proximanova-semibold text-base text-primary dark:text-dark-primary">{item.title}</Text>
                    <Text className="font-proximanova-semibold text-sm text-secondary dark:text-dark-secotext-secondary">{item.time}</Text>
                  </View>
                ))
              }
            ...
            <Text className="text-center font-proximanova-regular text-sm text-secondary dark:text-dark-secondary px-3">
              Earn this badge by working consisent hours over time. keep going to level up and earn rewardz
            </Text>
