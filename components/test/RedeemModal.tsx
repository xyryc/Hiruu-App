...
const ReddemModal = ({ visible, onClose, data }: any) => {
            ...
            <BlurView intensity={80} tint="dark" className="flex-1 justify-end">
                <View className="bg-white rounded-t-3xl">
                    ...
                    <SafeAreaView edges={["bottom"]} className="px-5 py-7">
                        <Text className="font-proximanova-semibold text-xl text-primary dark:text-dark-primary text-center"> Ready to Redeem? </Text>
                        ...
                        <Text className="font-proximanova-regular text-sm text-secondary dark:text-dark-secondary text-center px-9 mt-2.5"> This action will use your tokens to unlock the selected reward. Please confirm to proceed.</Text>
                        <View className="bg-[#EFF9FF]  rounded-xl p-2 w-full flex-row items-center -z-40 mt-5">
                            <Image source={data.img} contentFit="contain" style={{ width: 60, height: 60, margin: 15 }} />
                            <View className="-z-30">
                                <Text className="font-proximanova-semibold text-primary dark:text-dark-primary">{data.title}</Text>
                                <Text className="font-proximanova-regular text-secondary dark:text-dark-secondary text-sm mt-2.5">{data.subtitle}</Text>
                        ...
                        {data.tag !== "premium" && <View className="mt-4">
                            <BusinessDropdown
                                label="Select User"
                        ...
                        {data.tag === 'me' && <View className="mt-4 mx-1">
                            {
                                options.map((option, index) => (
                                    <TouchableOpacity onPress={() => setSelectedOption(option.value)} key={index} className="flex-row gap-2.5 mt-1.5">
                                        <View className={`h-4 w-4 rounded-full border-2 ${selectedOption === option.value ? 'border-[#4FB2F3]' : 'border-[#7A7A7A80]'} `} >
                                            <View className={`${selectedOption === option.value ? 'bg-[#4FB2F3] h-2 w-2 rounded-full m-auto' : ''}`}></View>
                                        </View>
                                        <Text className="font-proximanova-regular text-sm text-primary dark:text-dark-primary">{option.label}</Text>
                                    </TouchableOpacity>
                                ))
                            }
                        </View>}
